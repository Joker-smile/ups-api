...
class ShipFunction{
    ...
    public function allFunction($result)
    {
        // Start shipment
        $shipment = new Shipment();

        // Set shipper--发件人，必须
        $shipper = $this->shipSetShipper();
        $shipment->setShipper($shipper);

        // To address-收件人必须
        $shipTo = $this->shipSetShipTo($result);
        $shipment->setShipTo($shipTo);

        // From address--发自，必须
        $shipFrom = $this->shipSetShipFrom();
        $shipment->setShipFrom($shipFrom);

        // Sold to-- 销售方，当有发票的时候必须
        $soldTo = $this->shipSetSoldTo();
        $shipment->setSoldTo($soldTo);

        // Set service--服务方式，必须
        $service = $this->shipSetService();
        ...
        $internationlForms = $this->shipSetInvoice();
        ...
        private  function shipSetShipper()
        {
            // Set shipper--发件人，必须
            // Start shipment
            $shipment = new Shipment();
        ...
        private function shipSetShipTo($result)
        {
            // To address-收件人必须
        ...
        private function shipSetShipFrom()
        {
            // From address--发自，必须
            $address = new \Ups\Entity\Address();
        ...
        private function shipSetSoldTo()
        {
            // Sold to-- 销售方，当有发票的时候必须
            $address = new \Ups\Entity\Address();
        ...
        private function shipSetService()
        {
            // Set service--服务方式，必须
        ...
        private function shipSetInvoice()
        {
            //set invoice
        ...
        private  function shipAddPackage()
        {
            // Add Package--包裹，必须
